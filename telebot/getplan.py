...
def parser(user_id):  # это надо было все обернуть в функцию чтобы потом при импорте вызвать модуль.функция()
    ...
    try:
        ...
        error = f"Проблема с получением плана работ. Причины могут быть две: либо на сервере кратковременный сбой из-за " \
                f"перегруженности, нужно немного подождать, либо в базе указан неверный логин {dict_users.users[user_id]['tab_number']} " \
