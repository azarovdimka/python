...
def conversation(message):
            ...
            if changed(message.text) in changed(question):
                if 'скачать' in question:
                    ...
                    bot.send_message(message.chat.id, baza.dictionary[id]['answer'], reply_markup=general_menu(),
                                     parse_mode='Markdown')
                    bot.send_message(157758328,
                                     "2 - какая-то информация выдана не в строгом соответсвии по запросу: " + message.text)
                    found_result = True
        ...
        if len(results) < 8:  # выдает ответы при оптимальном количстве результатов
            for each_answer in results:  # TODO добавить исклюбчение если это место вашает программу
                bot.send_message(message.chat.id, each_answer, reply_markup=general_menu(),
                                 parse_mode='Markdown')  # TODO быть может для разгрузки памяти выдавать ответы генератором?
                bot.send_message(157758328,
                                 "3 - выдана информация из слов в случайном порядке по запросу: " + message.text)
                found_result = True

        if len(
                results) >= 8:  # не выдает ответы если их 8, крайне редко когда достигается, по другим методам поиска все равно сипит кучу ответов
    ...
    if not found_result:  # если ничего не найдено
        ...
        bot.send_message(157758328,
                         message.text)  # если запрос ненайден - бот об этом сообщит разрабочтику дублированием сообщения напрямую
        bot.send_message(message.chat.id,
