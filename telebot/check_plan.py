            ...
            for user_id in list_id:
                ...
                if '07:00' > current_time > '00:00' and not night_notify:  # обычно ['key'] выдает ошибку в некоторых местах нет ключа keyerror хотя ключ есть, а с методом get ключ видит
                    ...
                    try:
                        ...
                        #     path_plan = "/usr/local/bin/bot/plans/plan" + str(user_id) + ".txt"
                        ...
                        #                 bot.send_message(user_id, new_document, reply_markup=btn_doc, parse_mode='html')
                        #
                        #         else:
                        #             with open(doc_path, 'r', encoding='utf-8') as old_file:
                        #                 old_file = old_file.read()
                        #                 if new_document not in old_file:
                        #                     with open(doc_path, 'a', encoding='utf-8') as new_data:
                        #                         new_data.write(f"\n\n{new_document}")
                        #                         bot.send_message(user_id, new_document, reply_markup=btn_doc,
                        #                                          parse_mode='html')
                        #         bot.send_message(157758328, f'{fio} {new_document}', reply_markup=btn_doc,
                        #                          parse_mode='html')
                        #
                        #     sent_doc_counter += 1
                        ...
                        exc_event = exception_logger.writer(exc=exc, request="автоотправка плана ",
