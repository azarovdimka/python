...
def get_future_date():
    ...
    return future_month_int, days[future_month_int], future_month_big_name, future_year, \
           future_month_big_name_forbidden, future_month_big_name_forbidden_after_finish, \
           future_month_big_name_after_finish, future_month_int_after_finish
...
def check_true_date(message):
    ...
    future_month_int, future_days, future_month_big_name, future_year, \
    future_month_big_name_forbidden, future_month_big_name_forbidden_after_finish, \
    future_month_big_name_after_finish, future_month_int_after_finish = get_future_date()  # формирует путем прибавления определнного числа к введенному числу, чтобы сформировать желаему дату
...
permission_message = f'С 15.{from_month} по 03.{till_month} заказы принимаются на {future_month_big_name}.'
forbidden_message = f' Прием выходных на {future_month_big_name_forbidden} прекращен. ' \
                    f'Выходные на {future_month_big_name_after_finish} ' \
                    f'будут приниматься с 15.{from_month_aft_fin} по 03.{till_month_aft_fin}. \n ' \
                    f'Если Вам требуется отменить ранее заказанный выходной - обратитесь к своему начльнику ОКЭ.'
        ...
        if date:
            ...
            if free_days_before > 0:
                ...
                bot.send_message(message.chat.id, f'{name}, дата {date} успешно записана.',
                                 reply_markup=select_action())
                ordered_end = 'заказала' if surname[-1:] == 'а' or name[-1:] == 'я' else "заказал"
