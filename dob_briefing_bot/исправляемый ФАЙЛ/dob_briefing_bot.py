...
def check_asked_question(chosen_list, chosen_question, answered_list):
    """ Перед тем, как задать вопрос, смотрит был ли этот вопрос задан ранее. Если был задан, то выбирает другой вопрос.
    Как только находит незаданный вопрос - возвращает необходимый false."""
    ...
    with open('../static/AnimatedSticker.tgs', 'rb') as sti:
        ...
        if message.chat.id in krs_list:
            bot.send_message(message.chat.id,
                             f'Для того, чтобы выгрузить журнал проведения брифинга в любое время - отправьте сообщение '
                             f'"Выгрузить журнал"', reply_markup=select_action_krs())
        return
...
def conversation(message):
    ...
    def write_to_table(answer):
        ...
        tab_bp = handler_db.select_from_general_db(messagechatid=message.chat.id, item=tab_bp)

        # flight_number = flight_dict[message.chat.id]['flight_number']  # get
        flight_number = handler_db.select_from_general_db(messagechatid=message.chat.id, item=flight_number)
        ...
        handler_db.insert_new_flight(messagechatid=message.chat.id, id=str(id), date=reg_date, time_briefing=reg_time,
                                     flight_number=flight_number, aircraft=aircraft, tab_number_sb=tab_number_sb,
                                     fio_sb=fio_sb, tab_bp=tab_bp, fio_bp=fio_bp, question=question, answer=answer)
        return
